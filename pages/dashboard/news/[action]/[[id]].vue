...
const save = async (event) => {
  ...
  state.formData.body = tinyMCE.get("newsBody").getContent()
  if (state.formData?.id) {
    await axiosApi().put(apiPath.News.update, state.formData)
      .then(r => {
        common.showMessage('خبر با موفقیت ویرایش شد')
      }).catch(e => {
  ...
  if (Number(route.params.id))
    await axiosApi().get(apiPath.News.get.byId + route.params.id)
      .then(r => {
        ...
        tinymce.get('newsBody').setContent(state.formData.body);
  ...
  <v-form @submit.prevent="save" ref="verifyForm">
    <v-row>
      <v-col cols="12" md="8" lg="9">
        <v-card>
          <v-card-text>
            ...
            <!-- <v-textarea v-model="state.formData.body" id="newsBody" variant="outlined" placeholder="متن اصلی خبر..."
              rows="18"></v-textarea> -->
            <textarea id="newsBody" rows="10"></textarea>
      ...
      <v-col cols="12" md="4" lg="3">
        ...
        <v-card class="mb-5">
          <v-card-title>تصویر خبر</v-card-title>
          <v-divider></v-divider>
